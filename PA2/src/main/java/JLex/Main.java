package JLex; /**************************************************************
  ...
  Version 1.2.5, 7/25/99-5/16/00, [C. Scott Ananian]
   ...
   Rewrote internal JavaLexBitSet class as JLex.SparseBitSet for efficient
     unicoding.
   ...
   Rewrote JLex.CInput to fix bugs with line-counting in the specification file
  ...
  Notes:
    Simple optimization to the creation of the source files.
     ...
     field m_outstream of the class JLex.CLexGen.  This helps performance by
  ...
  Version 1.1.1, September 3, 1996 [Andrew Appel]
    Made the class "JLex.Main" instead of "JavaLex",
...
/******************************
  ...
  6) Clean up the JLex.CAlloc class and use buffered
  allocation.
...
class CEmit
{
  ...
  /*void emit_imports
      ...
      throws java.io.IOException      
	{
	  ...
	  if (JLex.CUtility.DEBUG)
	    {
	      JLex.CUtility.ASSERT(null != m_spec);
	      JLex.CUtility.ASSERT(null != m_outstream);
	    }*/
...
class CBunch
{
  /***************************************************************
    Member Variables
    **************************************************************/
  Vector m_nfa_set; /* Vector of JLex.CNfa states in dfa state. */
  SparseBitSet m_nfa_bit; /* BitSet representation of JLex.CNfa labels. */
  CAccept m_accept; /* Accepting actions, or null if nonaccepting state. */
  int m_anchor; /* Anchors on regular expression. */
  int m_accept_index; /* JLex.CNfa index corresponding to accepting actions. */

  /***************************************************************
    Function: JLex.CBunch
    Description: Constructor.
    **************************************************************/
  CBunch
    (
...
class CMakeNfa
{
  ...
  /***************************************************************
    Function: reset
    Description: Resets JLex.CMakeNfa member variables.
    **************************************************************/
  private void reset
    (
     )
  ...
  /***************************************************************
    Function: set
    Description: Sets JLex.CMakeNfa member variables.
    **************************************************************/
  private void set
    (
     CLexGen lexGen,
  ...
  /***************************************************************
    Function: thompson
    Description: High level access function to module.
    Deposits result in input JLex.CSpec.
    **************************************************************/
  void thompson
    (
     CLexGen lexGen,
...
class CMinimize
{
  ...
  /***************************************************************
    Function: fix_dtrans
    Description: Updates JLex.CDTrans table after minimization
...
class CNfa2Dfa
{
  ...
  void make_dfa
   ...
   /***************************************************************
    ...
    Description: Creates uncompressed JLex.CDTrans transition table.
    **************************************************************/
	...
	for (istate = 0; nstates > istate; ++istate)
	  {
	    ...
	    if (0 == m_spec.m_state_rules[istate].size())
	      {
		m_spec.m_state_dtrans[istate] = JLex.CDTrans.F;
		continue;
	      }
	    */
	    ...
	    /* JLex.Main loop of JLex.CDTrans creation. */
		...
		if (null == bunch.m_nfa_set)
		  {
		    ...
		    /*bunch.m_nfa_bit 
			= new JLex.SparseBitSet(m_spec.m_nfa_states.size());*/
		    bunch.m_nfa_bit = new SparseBitSet();
		  }
  ...
  /***************************************************************
    ...
    Description: Takes as input a JLex.CBunch with details of
    a dfa state that needs to be created.
    1) Allocates a new dfa state and saves it in 
    the appropriate JLex.CSpec vector.
    2) Initializes the fields of the dfa state
    with the information in the JLex.CBunch.
  ...
  private int add_to_dstates
	...
	/* Allocate, passing JLex.CSpec so dfa label can be set. */
	...
	/* Register dfa state using BitSet in JLex.CSpec Hashtable. */
...
class CAlloc
{
  ...
  /***************************************************************
    Function: newNLPair
    Description: return a new JLex.CNfaPair that matches a new
                 line: (\r\n?|[\n\uu2028\uu2029])
...
public class Main
{
  ...
  public static void main
	...
	if (arg.length < 1)
	  {
	    System.out.println("Usage: JLex.JLex.Main <filename>");
	    return;
	  }
...
class CLexGen 
{
  ...
  private boolean expandMacro
	...
	if (null == def)
	  {
	    /*JLex.CError.impos("Undefined macro \"" + name + "\".");*/
  ...
  /***************************************************************
    ...
    for them in state hashtable in JLex.CSpec structure.
  ...
  /********************************************************
    Function: packAccept
    Description: Packages and returns JLex.CAccept
  ...
  int advance
	...
	/*if (m_input.m_line_index > m_input.m_line_read) {
	  ...
	  JLex.CUtility.ASSERT(m_input.m_line_index <= m_input.m_line_read);
	}*/
...
final class SparseBitSet implements Cloneable {
     ...
     * which represent set bit indices in this JLex.SparseBitSet.
     */
    ...
    /**
     * Converts the JLex.SparseBitSet to a String.
     */
